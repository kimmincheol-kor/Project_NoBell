...
router.get('/', function (req, res, next) {
    const getRsSql = `SELECT * FROM nobell.restaurant_tbl WHERE rs_id=${req.user.owner_rs_id}`;
...
router.post('/', function (req, res, next) {
    ...
    if (req.user.owner_rs_id > 0) {
        const editRsSql = `UPDATE nobell.restaurant_tbl 
                                    ...
                                    WHERE rs_id=${req.user.owner_rs_id}`;

        (async (sql) => {
            try {
                const rows = await mysqlAPI(sql);
                res.status(200).send(`${req.user.owner_rs_id}`);
            } catch (err) {
                res.status(err).send();
            }
        })(editRsSql)
    ...
    // Register New Restaurant
        ...
        const regRsSql = `INSERT INTO nobell.restaurant_tbl(rs_name, rs_phone, rs_address, rs_intro, rs_open, rs_close, rs_owner) 
                                    VALUES("${req.body.rs_name}","${req.body.rs_phone}","${req.body.rs_address}","${req.body.rs_intro}",
                                            "${req.body.rs_open}","${req.body.rs_close}","${req.user.owner_email}")`;

        (async (sql) => {
            try {
                const rows1 = await mysqlAPI(sql); // Register

                const setRsIdSql = `UPDATE nobell.owner_tbl SET owner_rs_id=${rows1.insertId} WHERE owner_email="${req.user.owner_email}"`;
                req.user.owner_rs_id = rows1.insertId;
                
                await mysqlAPI(setRsIdSql); // Set New Rs_Id
...
router.post('/switch', function (req, res, next) {
    const switchRsSql = `UPDATE nobell.restaurant_tbl SET rs_state=${req.body.state} WHERE rs_id=${req.user.owner_rs_id}`;
